...
function connect(context, tunnelHost, dynoUser, privateKey) {
  var tunnelPort = 80
...
function _logConnectionError(err) {
  ...
  cli.error("Check that the dyno has started, and the tunnel is active by running:");
  cli.error(cli.color.white.bold("  heroku tunnels:status"));
  return err;
}
