...
function updateClientKey(context, heroku, configVars, callback) {
  if (context.flags.dyno &&
      (context.flags.dyno != _getExecDyno(configVars) && _getExecDyno(configVars) != "run.*" )) {
        cli.error(`Could not connect to dyno!\n` +
                  `${context.app} is connected on ${cli.color.white.bold(_getExecDyno(configVars))}\n` +
                  `To change this, run ${cli.color.magenta('heroku config:set HEROKU_EXEC_DYNO=' + context.flags.dyno)}`)
        cli.exit();
  }
